...
\begin{enumerate}
	...
	\item \textbf{\enquote{Limit the amount of time allowed for solving each task.}}
	      Integrating a limit like this into the tool we used for the questionnaire would have been difficult, but the tasks were comparatively simple and participants were aware that their completion time was measured, so we should hopefully have avoided the effects Wettel references here. \chmm
...
% FIXME: Use variables and null hypotheses above (or latter only in conclusion)
...
Note that the values are \gls{likert} responses to the question given in \cref{subsec:question}, meaning that \emph{lower} scores corresponds to \emph{higher} complexity.
...
Thus, from both a qualitative view (\ie, by looking at the \glspl{violin}) and a quantitative view (\ie, through the \glspl{mwu}) we cannot draw any significant conclusions on the difference in perceived complexity here and keep the null hypothesis. % TODO: Which?
...
\Cref{fig:sus} showcases the difference in measured \gls{sus} scores between \SEE{} and \gls{vscode}.
...
As the attentive reader may have guessed from the star in the heading, the difference here is significant when we apply the \gls{mwu}, specifically in favor of \gls{vscode} ($U = 114.5, p \approx 0.02116$).
The average \gls{sus} score for \SEE{} is $69.5$, with a median of $68.75$, while the \gls{sus} score for \gls{vscode} was on average $80.5$, with a median of $80.5$.
...
There are multiple possible reasons why \SEE{} does worse than \gls{vscode} here:
...
We can at least cast light on the last of these explanations by examining the feedback participants have left about \SEE{}, which we will do in the very next \cref{subsec:comments}.
...
Since our tested hypotheses might not be fully independent, we choose the variation by \textcite{benjamini2001} and fix the \gls{fdr} at our usual $\alpha$ of $0.05$.
We designate the $p$-value before fixing the \gls{multiplecomp} as $\xcancel{p}$.
	...
	\item \textbf{Experience with bigger software projects} and the \textbf{\gls{sus} for \gls{vscode}} correlate negatively with $\tau_b \approx -0.4225$ ($\xcancel{p} \approx 0.0313, p \approx 1$).
	      \begin{itemize}
		      \item This is a very surprising result to me.
		            I would have expected more experience on software project to have led to more familiarity with tools like \gls{vscode} and hence to higher usability ratings.
...
\begin{itemize}
	\item \textbf{Feature/change requests}:
	      \begin{itemize}
		      ...
		      \item It would be useful to have the feature to directly navigate to the \gls{base} instead of having to go from one parent to the next.
		            \answer{I agree when talking about \SEE{} as it is normally used, but for this study, the intent was also to have a non-trivial task that required moderate interaction with the \gls{city} environment.}
		      ...
		      \item A key combination to trigger edges for a given node would be useful to quickly navigate using edges.
		            \answer{As I write this, we are working on a new keyboard shortcut system that would accomodate this.}
	...
	\item \textbf{Miscellaneous comments and criticism}:
	      \begin{itemize}
		      ...
		      \item \SEE{} is getting very visually appealing, and the transitive edge animation reminds me of a stone skipping across water, which is very cool.
	      \end{itemize}
\end{itemize}
