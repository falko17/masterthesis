...
We will first go over the Ablauf %FIXME
before taking a look at the results themselves.
...
% TODO: Spelling of ironed correct?
...
We also take this opportunity to find any significant differences between the two groups $\alpha$ and $\beta$, to make sure any results in the dependent variables later on are not just due to such differences.
For this purpose, we define null hypotheses of the form \enquote{Variable $X$ is not different between groups $\alpha$ and $\beta$}, which we check with a two-sided \gls{mwu}, as we cannot assume normal distributions.
...
% TODO: Mention spotbugs excluded from bars


\begin{figure}
	\begin{center}
		\violinab{age}{Age}{15}{45}{3.1029}{1}
	\end{center}
...
\begin{figure}
	\begin{center}
		\violinab{total-time}{Time (in hours)}{0}{5}{0.1171}{0.106}
	\end{center}
...
\begin{figure}
	...
	\end{subfigure}
	\tikzexternalenable
	\caption{Number of respective answers to various demographic questions.}\label{fig:demobars}
\end{figure}
