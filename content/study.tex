...
\begin{enumerate}
	...
	\item \textbf{\enquote{Involve participants from industry.}}
	      Roughly half of our participants have reported working on bigger software projects (either within a company or as open-source contributions) for at least 3 years (see \cref{subsec:demographics}). \cyes
...
In total, we were able to fulfill 7/13 items on the wish list, with 3/13 being questionably fulfilled and 2/12 not being implemented.
...
Thus, from both a qualitative view (\ie, by looking at the \glspl{violin}) and a quantitative view (\ie, through the \glspl{mwu}) we cannot draw any significant conclusions on the difference in perceived complexity here and keep the null hypothesis $H_{d_0}$, that is, we keep the assumption that $A^c_V = A^c_S$.
...
Summarizing, we can reject $H_{c_0}$ and accept the alternative hypothesis $H_{c_1}$, specifically, that $S_V > S_S$.
