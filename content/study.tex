...
We choose these projects due to their language (Java is a well-known language and has a good \gls{ls} implementation), availability of source code, and size (not so high as to cause performance problems, but not so low as to be unrealistic).
...
For this reason, we will abstain from incorporating the \glsh{window}-related \glspl{capability} from \cref{sec:intowindow}.
Limiting ourselves, then, to the \glsh{city}-related changes from \cref{sec:intocity}, we have:
\begin{enumerate}
...
Outside of \SEE{}, there are a number of other \glsh{city} implementations, such as \emph{CodeCity}~\cite{wettel2007} or \emph{Software World}~\cite{knight2000} (see also the overview by \textcite{jeffery2019}).
In their evaluations, these papers often compare different platforms (such as Desktop to VR/AR)~\cite[\eg,][]{merino2017,fittkau2015, merino2018}, but as with the \SEE{}-related theses, we are most interested in those that have controlled experiments comparing a \glsh{city} tool with a traditional \gls{ide}.

One such study was done by \textcite{wettel2011} and compares \emph{CodeCity} against the Eclipse \gls{ide}, with the caveat that participants using Eclipse can also access an Excel spreadsheet of software metrics, as the \glsh{city} implementation would otherwise have an unfair advantage.
\citeauthor{wettel2011} based their study on an extensive survey of existing empirical work on software visualization, constructing a "wishlist" of desiderata for such studies~\cite[chapter 7]{wettel2011}.
...
\textcite{mehra2020} gave participants who were using Eclipse an additional 2D graph tool when evaluating the augmented reality \textsc{XRaSE} \glsh{city} visualization.
...
The collected data will be anonymized to make it impossible to identify any individual participants.
...
It has a validated\footnote{That is, the questionnaire was translated back into English by native speakers to check that the items have the intended meaning.
	See the comparison by \textcite{brix2023} for more information.}
German translation~\cite{reinhardt2015}, is the most used \gls{poststudy} questionnaire for this purpose~\cites[1615]{sauro2009}[577]{lewis2018}, and is very well-suited for comparing two systems or interfaces~\cites[195]{peres2013}[590--591]{bangor2008}.
It consists of ten \glsh{likert} questions which alternate between positive and negative aspects of the system and returns a score between 0 and 100, representing the system's usability~\cite{brooke1996}.
...
\begin{itemize}
	...
	\item ask questions with numerical, text-based, selection, and \glsh{likert}-based input, and
...
It compares a desktop \glsh{city} implementation against a traditional \gls{ide} along with providing tabular metrics to make the comparison fair.
...
We can neither assume normal distributions for our data, nor is it always interval-scaled, so we choose the \gls{mwu} to determine statistically significant differences between either our two groups $\Psi$ and $\Omega$ or between the two tools \SEE{} and \gls{vscode} (with the exception of the \gls{sus}, see \cref{subsec:usability}).
...
There have been \participants participants in total---there was originally one more, but we had to exclude him from most analyses because he indicated he had no knowledge of Java.
Participants were gained via convenience sampling, that is, I asked fellow students, colleague developers from Axivion\footnote{
	A company producing static analysis tools, such as the original \gls{gxl} generation program used in \SEE{}.
}, and other acquaintances who had software engineering experience to participate.
...
For JabRef, unit tests were all centralized under the \tt{src.test} Java package, so I had expected there to be less issues here for task $B$ than with SpotBugs.
...
We can see a slight, but non-significant advantage for \gls{vscode} for task $A$ (\cref{fig:a1t,fig:a2t}), where the classes with the highest \glsentrylong{nom} had to be identified.
\gls{vscode} users were allowed to use a table that could be sorted by exactly this metric, while \SEE{} users had to glean this information by inspecting the color of the buildings in the \gls{city}.
...
Here, participants using \gls{vscode} outperformed those using \SEE{}, with an especially clear difference for task $C_1$, where we have a $p$-value of ca.\ $0.001$.
...
\subsection[Usability]{$\bigstar$ Usability}\label{subsec:usability}
...
Note that the values are \glsh{likert} responses to the question given in \cref{subsec:question}, meaning that \emph{lower} scores correspond to \emph{higher} complexity.
...
Because the data contains paired samples, we choose the \gls{wilcoxon} as our statistical test, as the \gls{mwu} only works for independent samples.
...
As one might guess from the star in the heading, the difference here is significant when we apply the \gls{wilcoxon}, specifically in favor of \gls{vscode} ($T = 27.5, p \approx 0.02024$).
...
For the correlations themselves, we use Kendall's coefficient of rank correlation $\tau_b$~\cite{kendall1938, kendall1945}, as we compare ordinal\footnote{
	With the exception of the continuous \emph{age} variable, where we could have used Pearson's correlation coefficient $r$~\cite{bravais1844, pearson1895} or Spearman's $r$~\cite{spearman1904}, but Kendall's coefficient is more robust and works for this kind of comparison too.
	This also makes the individual correlations more comparable to one another.
} independent with continuous dependent variables.
For this statistic, a value of $1$ indicates a perfectly identical ranking, $0$ indicates no relationship, and $-1$ indicates a completely different ranking.
	...
	\item \textbf{Feature/change requests}:
	      \begin{itemize}
		      ...
		      \item It would be useful to have the feature to directly navigate to the \gls{base} instead of having to go from one parent to the next.
		            \answer{I agree when talking about \SEE{} as it is normally used, but for this study, the intent was to have a non-trivial task that required moderate interaction with the \glsh{city} environment.}
		      ...
		      \item A less three-dimensional view may have been useful to navigate this city more easily.
		            \answer{This may be true here, since we have not used all three dimensions for metrics, but at its core, \SEE{} is a 3D \glsh{city} visualization.}
	      \end{itemize}
...
It is probably more common to encounter a setup where such information is integrated into the \gls{ide}, for example, in the form of a \glsh{smell} tool that tells the developer when components exceed certain metrics.
...
Based on these data, we can answer \textsf{RQ3} as follows:
...
There, I compared \SEE{} against the Axivion Dashboard, a web-based tabular \glsh{smell} explorer, with the result being that using the Dashboard leads to higher correctness, but using \SEE{} leads to higher speed and a better \gls{asq} effort rating.
...
As said before, this may be due to the choice of \gls{ide} (the most-used \gls{ide} by other studies has been Eclipse, while we used \gls{vscode}), choice of \glsh{city} implementation, or choice of tasks.
Further investigation into the exact causes here---perhaps by rigorously comparing various \glspl{ide}, \glsh{city} tools, tasks, and so on, against each other in further user studies---could be a good avenue for future research.

\chapterendornament{78}{0.2}
