...
\begin{itemize}
	...
	\item \Glspl{window} have previously been made editable by \textcite{moritz} in his bachelor's thesis, also enabling collaborative editing over the internet.
	      I unfortunately had to remove these changes because they did not work anymore in the current version of \SEE{}, and additionally caused a lot of complexity overhead in the \glsh{window} implementation that would have made the \gls{lsp} integration much harder.
...
The naïve solution used in \cref{alg:generate}---simply going over all nodes each time to find the best fit---leads to an unacceptable runtime.
...
We then associate diagnostics to nodes using the \glsh{intervaltree}-based function described in \cref{subsec:kd}.
...
There were only minimal changes, such as the integration of new icons for the different \gls{lsp} diagnostic types, and a change that allows leaf erosion icons to be displayed on non-leaf nodes, since the \gls{lsp} diagnostics are not always present at the lowest level in the hierarchy.
...
Some of these functionalities will seem quite similar to the way the corresponding \glsh{city} \gls{lsp} features were implemented in the previous section.
...
Only the algorithm as described in the paper here was part of the evaluation, any other layouting or drawing done by \SEE{} has been excluded from the analysis.
...
For the generation, we enabled every edge type, node type, and diagnostic, except for JabRef and SpotBugs, where we only enabled some node types (namely File, Module, Namespace, Package, Class, and Interface) and only one edge type (Extend for JabRef and Reference for SpotBugs), otherwise the generation would have taken much too long (multiple hours or even days).
\Cref{tab:benchmark} lists all projects along with their node and edge count, \gls{loc}, and average generation time.
...
\begin{table*}[htbp]
	...
	\begin{tabular}{@{}llrrrrr@{}}
		\toprule
		\multicolumn{1}{c}{\multirow{2}{*}{\textbf{Name}}} & \multicolumn{1}{c}{\multirow{2}{*}{\textbf{\Gls{ls}}}} & \multicolumn{1}{c}{\multirow{2}{*}{\textbf{k\gls{loc}}}} & \multicolumn{1}{c}{\multirow{2}{*}{\textbf{\# Nodes}}} & \multicolumn{1}{c}{\multirow{2}{*}{\textbf{\# Edges}}} & \multicolumn{2}{c}{\textbf{Average Time [m:s]}}                         \\
...
This difference cannot be explained by the idea that edges may not play a big role in generation time after all, since a closer look at the data for JabRef and SpotBugs reveals a much longer generation time for SpotBugs, even though SpotBugs has fewer nodes (but a lot more edges) than JabRef.
...
The generation of edges is by far the most expensive part\footnote{
	As the distribution of other components is hard to tell, there is another version of the diagram without the "edges" component in \cref{app:techeval}.
	The same appendix also contains a "normalized" version, with the node and edge components divided by the number of nodes and edges, respectively.
...
\begin{figure}[tbhp]
	...
	\caption{The relative improvement of the optimized algorithm by number of edges (\ie, the percentage by which the time is reduced when using \cref{alg:interval} instead of the base \cref{alg:generate}).
		Error bars are only shown when their extent exceeds the diameter of the point, and describe the full range of datapoints (from minimum to maximum).\\
...
I suspect that the hypothesis is still directionally correct, but that it does not hold when making comparisons across multiple \glspl{ls}, since the performance differences between their implementation makes too big a dent.

We can additionally check whether the weak form of the hypothesis (\ie, asserting a weakly monotonic relationship) is true:
Calculating Kendall's coefficient $\tau_b$~\cite{kendall1938, kendall1945} for the relative time improvement and the number of edges, we get a value of $\tau_b = 0.8\overline{6}$, which indeed indicates a strong positive relationship.
Hence, we can conclude that, in general, the more edges there are, the more the optimization implemented in \cref{alg:interval} helps.
However, more extensive tests across a wide breadth of \glspl{ls} would be required to solidify this point further.
...
My guess is that there is a mismatch between Rust modules and files due to the Rust Analyzer, that is, module nodes are separate from the files that contain them.
...
\Crefrange{sec:generate}{sec:intowindow} specifically provide an answer to the first research question \textsf{RQ1}.
...
So, while it is feasible to generate \glspl{city} using the \glsentrylong{lsp}, more research and work needs to be done for this to work well with very large software projects.
