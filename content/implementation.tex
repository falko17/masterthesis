...
\begin{itemize}
	\item I have integrated the OmniSharp \gls{lsp} C\# library~\cite{csharplanguageserverprotocol2025} into \SEE{}, which we will leverage in the subsequent sections so that we can use \gls{lsp} without needing to worry about \gls{jrpc} encoding, data models, and so on.
...
\fxnote{Improve coloring of inner boxes.}
...
In our case, we can create the tree by constructing a 2d-tree\footnote{
	Note that, in the actual implementation, the construction of the \gls{kdtree} (excluding its augmentation to an \gls{intervaltree}) is handled by the external \tt{Supercluster.KDTree} library~\cite{regina2024}.
...
Some examples of \glspl{city} rendered using this algorithm are given in \cref{fig:gencities} in \cref{fig:techeval}.
