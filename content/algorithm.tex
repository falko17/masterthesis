...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State $a' \gets \emptyset$
		\State $a'(v, \tt{Source.Name}) \gets s\tt{.name}$
		\State $a'(v, \tt{Source.Path}) \gets d$
		\State $a'(v, \tt{Type}) \gets s\tt{.kind}$
		\State $a'(v, \tt{Deprecated}) \gets (\text{deprecated} \in s\tt{.tags})$
		\State $a'(v, \tt{Source.Range}) \gets s\tt{.range}$
		\State $a'(v, \tt{Metric.LOC}) \gets e_l^{s\tt{.range}} - b_l^{s\tt{.range}}$
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\Statex $\hphantom{B \gets \{ v \in V \mid a(v, \tt{Path}) = d} \land\ b_c^r \geq b_c^{\text{getR}(v)} \land
		...
		\If{$(v, c\tt{.severity}) \in \text{dom}(a)$}
		\State $a(v, c\tt{.severity}) \gets a(v, c\tt{.severity}) + 1$
		\Else
		\State $a(v, c\tt{.severity}) \gets 1$
		\EndIf
		\EndFor
		\EndFor
		\EndFunction
